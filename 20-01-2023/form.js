...
const funzioneDelete = (callType, idProduct) => {
  fetch(`https://api.escuelajs.co/api/v1/`+ callType + `/` + idProduct, {
   ...
   .then(data => {
    if (data !== true) {
      alert(
      `${callType} Prodotto non eliminato: `  + data.name);
    } else{
      alert (`${callType} Prodotto eliminato:`);
...
deleteProductForm.addEventListener('submit', (event) => {
  event.preventDefault();
  const deleteProduct = inputIdProduct.value
  funzioneDelete("products", deleteProduct);
});
...
//       images: [inputImages.value],
//     };
//     funzionePut(newObj,"products", idProduct);
//     alert("Hai modificato un prodotto");
//   console.log(newObj);
// });

// const funzionePut = (objBody, idProduct) => {
// fetch(`https://api.escuelajs.co/api/v1/` + idProduct , {
//   method: "PUT",
//   headers: {"Content-Type": "application/json"},
//   body: JSON.stringify(objBody),
//  })
...
//   if(data.statusCode <= 400 && data.statusCode < 500){
//     alert(
//     `${idProduct} Prodotto non modificato: `  +  data.message[0]);
//   } else{
//     alert (`${idProduct} Prodotto modificato:`);
